...
from sky_dataset_unconditional import (
    SkyDatasetUnconditional,
)
...
parser.add_argument("--save_image_epochs", default=5, type=int)
    ...
    for epoch in range(config.num_epochs):
        ...
        if accelerator.is_main_process:
            ...
            if (epoch + 1) % config.save_model_epochs == 0 or epoch == config.num_epochs - 1:
                torch.save(
                    model.state_dict(),
                    f"{config.output_dir}/models/model{epoch + 1}.pt",
                )
                if config.push_to_hub:
                    upload_folder(
                        repo_id=repo_id,
